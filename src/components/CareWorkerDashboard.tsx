...
const CareWorkerDashboard = ({ mainUser }: UserProfile) => {
  ...
  const handleClockAction = async (
    ...
    try {
      ...
      message.error("An error occurred during clock action.");
    }
  };
  ...
  // Pie chart data for current clock status (real data)
  const clockInCount = currentlyClockedIn ? 1 : 0;
  const clockOutCount = currentlyClockedIn ? 0 : 1;

  const clockStatusData = {
    labels: ["Clocked In", "Clocked Out"],
    datasets: [
      {
        data: [clockInCount, clockOutCount],
  ...
  const clockStatusOptions = {
    responsive: true,
    plugins: {
      legend: { position: "bottom" as const },
      title: { display: true, text: "Your Current Clock Status" },
    },
  };
